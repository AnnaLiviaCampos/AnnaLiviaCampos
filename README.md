
<!-- ==================== HEADER ==================== -->

<div align="center">

# Hello World! 👋

### Olá, eu sou a Anna Lívia! | Hi, I'm Anna Lívia!

**Análise e Desenvolvimento de Sistemas | Systems Analysis and Development**

📍 Brasil | Brazil

</div>

---

<!-- ==================== ABOUT ME ==================== -->

## 👩‍💻 Sobre mim | About Me

### 🇧🇷 Português

Olá! Sou Anna Lívia, estudante de **Análise e Desenvolvimento de Sistemas (ADS)** no Instituto Federal de São Paulo (IFSP) – Campus Boituva.

- 🎓 Atualmente, estou cursando o segundo semestre de ADS.
- 💻 Estou desenvolvendo meus conhecimentos em programação, desenvolvimento web, banco de dados e sistemas operacionais.
- 🌱 Estudando C, TypeScript, HTML, CSS e SQL.
- 🔭 Tenho experiência profissional na área da Qualidade, com atuação em gestão de fornecedores, análise de não conformidades e melhoria de processos.
- 🚀 Busco aprimorar minhas habilidades técnicas e desenvolver projetos que conectem tecnologia e soluções para problemas reais.

### 🇺🇸 English

Hello! I'm Anna Lívia, a **Systems Analysis and Development** student at the Federal Institute of São Paulo (IFSP), Boituva Campus, Brazil.

- 🎓 Currently in my second semester of college.
- 💻 Developing my knowledge in programming, web development, databases, and operating systems.
- 🌱 Learning C, TypeScript, HTML, CSS, and SQL.
- 🔭 I have professional experience in Quality Management, including supplier management, nonconformity analysis, and process improvement.
- 🚀 I'm focused on improving my technical skills and building projects that connect technology with real-world solutions.

---

<!-- ==================== MORE ABOUT ME ==================== -->

<details>
  <summary><b>✨ Mais sobre mim | More About Me</b></summary>

### 🇧🇷 Português

Minha trajetória profissional começou na área de Compras e evoluiu para a área da Qualidade, onde desenvolvi habilidades de análise, organização, comunicação e resolução de problemas.

Atualmente, concilio minha experiência profissional com os estudos em tecnologia, buscando aplicar os conhecimentos adquiridos em projetos acadêmicos e pessoais.

### 🇺🇸 English

My professional journey began in the Purchasing department and evolved into Quality Management, where I developed analytical, organizational, communication, and problem-solving skills.

I currently combine my professional experience with my technology studies, applying what I learn to academic and personal projects.

</details>

---

<!-- ==================== TECH STACK ==================== -->

## 💻 Tecnologias e Ferramentas | Tech Stack

### Linguagens de Programação | Programming Languages

<p>
  <img alt="C" height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/c/c-original.svg">
  &nbsp;
  <img alt="TypeScript" height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg">
</p>

### Desenvolvimento Web | Web Development

<p>
  <img alt="HTML5" height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg">
  &nbsp;
  <img alt="CSS3" height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg">
  &nbsp;
  <img alt="Node.js" height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg">
</p>

### Banco de Dados | Databases

<p>
  <img alt="SQL" height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg">
</p>

### Ferramentas | Tools

<p>
  <img alt="VS Code" height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg">
  &nbsp;
  <img alt="Git" height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg">
  &nbsp;
  <img alt="GitHub" height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg">
  &nbsp;
  <img alt="Linux" height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linux/linux-original.svg">
  &nbsp;
  <img alt="Postman" height="40" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postman/postman-original.svg">
</p>

---

<!-- ==================== PROJECTS ==================== -->

## 📂 Projetos | Projects

### 🇧🇷 Português

Aqui você encontrará projetos acadêmicos e pessoais desenvolvidos durante minha jornada em tecnologia.

### 🇺🇸 English

Here you will find academic and personal projects developed throughout my journey in technology.

<!-- Adicione seus projetos aqui. -->
<!-- Add your projects here. -->

---

<!-- ==================== SPOTIFY ==================== -->

## 🎧 O que estou ouvindo | Currently Listening

<div align="center">

<a href="https://github.com/kittinan/spotify-github-profile">
  <img
    src="https://spotify-github-profile.kittinanx.com/api/view?uid=31jcythqde6wkljfbt2oe5rhgqa4&cover_image=true&theme=spotify-embed&show_offline=false&background_color=121212&interchange=false&profanity=false&hide_remaster=false&mode=light&bar_color=bd4c83&bar_color_cover=false"
    alt="Spotify GitHub Profile"
  />
</a>

</div>

---

<!-- ==================== CONTACT ==================== -->

## 📫 Contato | Connect With Me

<div align="center">

<a href="https://www.instagram.com/annaliviacamp/">
  <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" alt="Instagram">
</a>

<!-- Adicione seu LinkedIn aqui. -->
<!-- Add your LinkedIn profile here. -->

</div>

---

<!-- ==================== FOOTER ==================== -->

<div align="center">

### ✨ "Transformando conhecimento em soluções."

### ✨ "Turning knowledge into solutions."

</div>
